...
function HomeScreen({ onScan, onSearch, onHeritage }: { onScan: () => void; onSearch: () => void; onHeritage: (qrValue?: string) => void }) { return <div className="screen cream"><div className="home-scroll"><Header/><div className="home-main"><div className="home-copy"><small>DISCOVER</small><h1>정교한 디자인의<br/>가치를 발견하세요</h1></div><button className="scan-orbit" onClick={onScan}><span><Camera size={32}/><b>제품 스캔하기</b></span></button><button className="text-link" onClick={onSearch}>또는 제품명으로 직접 검색하기</button></div><section className="recent"><small>RECENTLY VIEWED</small><h2>최근 조회한 제품</h2><div className="cards"><ProductCard image={bagImage} title="MCM 비세토스 보스턴백" date="2026. 08. 12" detail="Visetos Heritage Canvas" onClick={() => onHeritage("KOY-001")}/><ProductCard image={backpackImage} title="KOY 아티산 월렛" date="2026. 08. 10" detail="Capstone Demo Archive" onClick={() => onHeritage("KOY-002")}/></div></section></div><BottomNav/></div>; }
